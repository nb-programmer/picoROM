 ...
 *    inputhandler: an optional function to handle unexpected input before the
 * transfer
 *
 * If message is provided, it will be printed every few seconds until the
 * first data is received.
 *
 * If inputhandler is provided, it will be called if unexpected input is
 * received before the transfer begins.  The unexpected input character will be
 * passed to the handler, which should return 'true' if it handled it.
 */
